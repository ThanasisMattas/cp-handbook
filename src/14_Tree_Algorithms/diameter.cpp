...
void solve_highest_point(int u, int u_prev)
{
  ...
  if ((adj[u].size() == 1) && (adj[u][0] == u_prev)) {
    ...
    for (auto u_next : adj[u]) {
      ...
      if (to_leaf[u_next] + 1 > first_max) {
        second_max = first_max;
        first_max = to_leaf[u_next] + 1;
        to_leaf[u] = first_max;
      } else if (to_leaf[u_next] + 1 == first_max) {
        second_max = first_max;
      } else {
        second_max = max(second_max, to_leaf[u_next] + 1);
      }

      max_len[u] = first_max + second_max;
    }
  }
}
...
{
  // In order to be a leaf, the single neighbor has to be u_prev.
  if ((adj[u].size() == 1) && (adj[u].front() == u_prev)) {
    if (to_leaf[u_prev] == 0) {
      to_leaf[u_prev] = 1;
      leaf[u_prev] = u;
    }
